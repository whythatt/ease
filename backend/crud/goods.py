...
async def give_goods(redis_key: str, image_url: str, page_index: int):
    ...
    if goods_data == []:
        ...
        goods_data = goods_data.get("products", [])
    else:
...
async def get_goods_by_url(page_index: int, image_url: str) -> GoodsResponseSchema:
    redis_key = f"products:{hashlib.md5(image_url.encode()).hexdigest()[:12]}"
